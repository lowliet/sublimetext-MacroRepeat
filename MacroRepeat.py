...
class MacroRepeatCommand(sublime_plugin.TextCommand):
    def run(self, edit):
    ...
    def __execute(self, text):
        if not text.isdigit() and len(text) > 0:
            print("Macro Repeat | Wrong number")
        elif len(text) > 0 and int(text) > (self.__get_last_line() - self.__get_current_line()):
            print("Macro Repeat | Number too big (bigger than number of lines in file)")
        else:
